...
class TempCNN(ClassificationModel):
    def __init__(self, input_dim, nclasses, sequence_length, kernel_size=5, hidden_dims=64, dropout=0.5, response=None):
        ...
        if response == "classification":
            self.outputlayer = nn.Sequential(nn.Linear(4 * hidden_dims, nclasses), nn.LogSoftmax(dim=-1))
        elif response == "regression_relu":
            self.outputlayer = nn.Sequential(nn.Linear(4 * hidden_dims, 1), nn.ReLU())
        elif response == "regression_sigmoid":
            self.outputlayer = nn.Sequential(nn.Linear(4 * hidden_dims, 1), nn.Sigmoid())
        else:
    ...
    def forward(self,x):
        ...
        x = self.flatten(x)
        x = self.dense(x)
        return self.outputlayer(x), None, None, None
