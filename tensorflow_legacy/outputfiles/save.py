...
#-----------------------------------------------------------------------
def write_predictions_csv(test_file, p_test, y_test, regression):
	"""
	...
	if  len(p_test.shape)==1 and regression==False: #-- saving class only [integer]
		np.savetxt(test_file, p_test.astype(int), delimiter=',', fmt='%i')
		np.savetxt(test_file.replace("test","ref"), y_test.astype(int), delimiter=',', fmt='%i')
	else: #saving proba [float]
		np.savetxt(test_file, p_test, delimiter=',', fmt='%1.6f')
		np.savetxt(test_file.replace("test","ref"), y_test, delimiter=',', fmt='%1.6f')
