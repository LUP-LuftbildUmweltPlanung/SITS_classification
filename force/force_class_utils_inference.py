...
def force_class_pre(preprocess_params,aoi):
    ...
    if preprocess_params["hold"] == True:
        subprocess.run(['xterm','-hold','-e', cmd], stderr=subprocess.DEVNULL)
    else:
        subprocess.run(['xterm', '-e', cmd], stderr=subprocess.DEVNULL)
    ...
    X_TILES, Y_TILES = extract_coordinates(f"{temp_folder}/{project_name}/FORCE/{basename}/tile_extent.txt")

    return X_TILES, Y_TILES

def force_class(preprocess_params, aoi, DATE_RANGE, X_TILE_RANGE, Y_TILE_RANGE):
    force_dir = f"{preprocess_params['force_dir']}:{preprocess_params['force_dir']}"
    local_dir = f"{os.sep + preprocess_params['process_folder'].split(os.sep)[1]}:{os.sep + preprocess_params['process_folder'].split(os.sep)[1]}"
    scripts_skel = f"{os.path.dirname(os.path.dirname(os.path.abspath(__file__)))}/force/skel"
    force_skel = f"{scripts_skel}/force_cube_sceleton"
    ...
    replacements = {
        ...
        f'X_TILE_RANGE = 0 0':f'X_TILE_RANGE = {X_TILE_RANGE} {X_TILE_RANGE}',
        f'Y_TILE_RANGE = 0 0':f'Y_TILE_RANGE = {Y_TILE_RANGE} {Y_TILE_RANGE}',
